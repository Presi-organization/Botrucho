...
export default class SiataCommand extends ICommand {
  ...
  async execute(interaction: CommandInteraction, guildDB: IGuildData): Promise<void> {
    ...
    try {
      ...
      const mapCropped = await this._getCroppedImage(mapImage as JimpInstance, mapInfo);
      const radarCropped = await this._getCroppedImage(radarImage as JimpInstance, radarInfo);
  ...
  private async _getCroppedImage(image: JimpInstance, cropInfo: CropInfo) {
