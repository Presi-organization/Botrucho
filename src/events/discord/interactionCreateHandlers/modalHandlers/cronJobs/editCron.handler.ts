...
export class EditCronHandler implements IIntCreateHandler {
  ...
  private _validateEmojis = (text: string): { isValid: boolean; message?: string } => {
    ...
    // Regex for standard Unicode emoji characters
    // This covers most standard emojis including skin tone and gender variations
    const emojiRegex = /(\p{Emoji_Presentation}|\p{Emoji}\uFE0F)/gu;

    // Regex for Discord custom emojis format <:name:id> or <a:name:id> for animated
    const discordEmojiRegex = /<a?:[a-zA-Z0-9_]+:[0-9]+>/g;

    // Create a clean string without valid emojis
    let cleanText: string = text.replace(emojiRegex, '').replace(discordEmojiRegex, '');
